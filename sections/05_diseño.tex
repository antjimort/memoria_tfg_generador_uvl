...
\begin{figure}[H]
    \centering
    \makebox[\textwidth][c]{%
        \includegraphics[width=1.4\textwidth]{figura_5_1_modelo_conceptual.pdf}
    }
...
\subsubsection*{FeatureModel generado}
...
\begin{itemize}
  ...
  \item \textbf{Reproducibilidad mediante el uso de semillas:} El usuario puede establecer la semilla utilizada para controlar la aleatoriedad del proceso de generación. Esta decisión permite repetir una ejecución utilizando la misma semilla y los mismos parámetros de configuración, siempre que se mantengan la versión del generador y las condiciones de ejecución. De esta forma, se facilita la reproducción de experimentos, la comparación de resultados y el análisis de posibles errores. Para ello, la clase FmgeneratorModel incorpora el atributo seed como parte de la configuración del generador.
\end{itemize}
...
La \textbf{capa de dominio/lógica} está formada por la lógica interna del generador desarrollado siguiendo la arquitectura de extensiones de Flamapy, junto con los servicios encargados de gestionar la configuración dentro de UVLHub. Por un lado, la herramienta contiene el modelo de configuración y la lógica necesaria para producir modelos de características expresados en UVL. Por otro lado, los servicios se encargan de reconstruir y validar los parámetros recibidos entre los diferentes pasos del generador mediante las \textit{routes}, garantizando que la información utilizada durante la generación sea coherente.
...
\begin{enumerate}
    ...
    Tras cada paso, las rutas y los servicios internos de UVLHub comprueban los parámetros recibidos y actualizan el diccionario de configuración almacenado en la sesión de Flask. Estas comprobaciones permiten detectar valores inválidos antes de continuar con el asistente. En caso de error, se informa al usuario para que corrija los parámetros correspondientes. En esta fase todavía no se construye la instancia de \texttt{FmgeneratorModel}.

    \item \textbf{Obtención de la configuración y preparación de Pyodide:}
    En el último paso del asistente, el navegador obtiene la configuración en formato JSON mediante el endpoint \texttt{/generator/random/params-json}. Antes de ejecutar el generador, se prepara el entorno de Pyodide y se cargan las dependencias necesarias para ejecutar el código Python en el navegador mediante WebAssembly.

    \item \textbf{Construcción de la configuración y ejecución del generador:}
    Dentro de Pyodide, el archivo \texttt{fmgen\_wrapper.py} utiliza el método \texttt{from\_flat\_dict} para construir la instancia de \texttt{FmgeneratorModel} a partir de los parámetros recibidos. Una vez validada la configuración, se ejecuta la operación \texttt{GenerateFeatureModel} y los modelos obtenidos se transforman a formato UVL. Si se produce un error durante este proceso, se informa al usuario de que no ha sido posible completar la generación.
...
El flujo de datos del sistema comienza con los parámetros introducidos por el usuario mediante la interfaz basada en \textit{wizard}. Las rutas del módulo \texttt{generator} de UVLHub reciben estos datos y, junto con los servicios internos, realizan las comprobaciones correspondientes a cada paso. La configuración se mantiene en forma de diccionario dentro de la sesión de Flask, permitiendo conservar los valores introducidos durante el recorrido por el asistente.

Una vez completada la configuración, el navegador obtiene estos parámetros en formato JSON mediante el endpoint \texttt{/generator/random/params-json}. Dentro del entorno de ejecución proporcionado por Pyodide, el archivo \texttt{fmgen\_wrapper.py} utiliza el método \texttt{from\_flat\_dict} para construir una instancia de \texttt{FmgeneratorModel}. Esta instancia se utiliza para ejecutar la operación \texttt{GenerateFeatureModel}, que produce los objetos \texttt{FeatureModel}. Posteriormente, estos se transforman a formato UVL y se ponen a disposición del usuario para su visualización o descarga. De esta forma, la construcción del modelo de configuración y la generación se realizan en el navegador, de acuerdo con el requisito RNF-03.
...
La representación intermedia de la configuración consiste en un diccionario de parámetros almacenado temporalmente en la sesión de Flask. Este diccionario permite mantener los valores introducidos por el usuario entre los diferentes pasos del \textit{wizard}, sin construir todavía una instancia del modelo de dominio del generador.

Las rutas y los servicios del módulo \texttt{generator} gestionan y comprueban los parámetros correspondientes a cada paso. Cuando el navegador solicita la configuración, el servidor proporciona su representación en formato JSON. A partir de esta información, \texttt{fmgen\_wrapper.py} construye la instancia de \texttt{FmgeneratorModel} mediante \texttt{from\_flat\_dict} dentro de Pyodide, en el navegador del usuario.
...
Esta representación se utiliza para mantener el estado temporal del asistente y transferir los parámetros al entorno de generación del navegador. No constituye un registro permanente de las configuraciones utilizadas ni de los modelos obtenidos.
...
El mapeo entre el diccionario de parámetros y el modelo interno de configuración se realiza en el navegador del usuario. Una vez recibida la configuración en formato JSON, \texttt{fmgen\_wrapper.py} utiliza el método \texttt{from\_flat\_dict} de \texttt{FmgeneratorModel} para convertir los parámetros en los atributos y objetos de configuración asociados. Esta transformación permite pasar de la representación utilizada durante el \textit{wizard} a la representación orientada a objetos que necesita el generador.
...
Una vez construida la instancia de \texttt{FmgeneratorModel}, la configuración se valida y se utiliza para ejecutar la operación \texttt{GenerateFeatureModel} dentro de Pyodide. El objeto \texttt{FeatureModel} obtenido se transforma posteriormente a una representación textual en UVL. Tanto la construcción de la configuración como la ejecución del generador tienen lugar en el navegador.
